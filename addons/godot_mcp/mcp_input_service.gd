...
func _dispatch_event(event: InputEvent, event_data: Dictionary = {}) -> void:
	...
	# Mouse button events may also be consumed by UI controls (buttons, etc.)
	if not force_unhandled and (event is InputEventMouseMotion and event.button_mask != 0 or event is InputEventMouseButton):
